...
tcgaCompare = function(maf, capture_size = NULL, cohortName = NULL, primarySite = FALSE, col = c('gray70', 'black'), medianCol = 'red', fn = NULL, width = 8, height = 5, fontSize = 10){
  ...
  tcga.cohort$pos = as.numeric(as.character(factor(x = tcga.cohort$cohort, levels = levels(tcga.cohort$cohort), labels = 1:length(levels(tcga.cohort$cohort)))))
  tcga.cohort = lapply(split(tcga.cohort, f = as.factor(tcga.cohort$cohort)), function(x){
                      x[,pos_updated := seq(unique(x[,pos])-1, unique(x[,pos]), length.out = nrow(x))]
                      x
                    })
  tcga.cohort = data.table::rbindlist(tcga.cohort)
  tcga.cohort$color = ifelse(test = tcga.cohort$cohort %in% cohortName, yes = col[2], no = col[1])
  tcga.cohort[,log_rate := log10(total)]

  par(mar = c(5, 3, 2, 1))
  yt = seq(floor(min(tcga.cohort[,log_rate])), ceiling(max(tcga.cohort[,log_rate])), 1)
  plot(tcga.cohort$pos_updated, tcga.cohort$log_rate, axes = FALSE, pch = 16, cex = 0.6, xlab = '',
       ylab = '', col = tcga.cohort$color, ylim = c(min(yt), max(yt)))
  axis(side = 1, at = unique(tcga.cohort[,pos])-0.5, labels = levels(tcga.cohort[,cohort]),
       las = 2, font = 2, col = 'white', col.ticks = 'black', lwd.ticks = 2, lwd = 2)

  #abline(v = seq(0, max(tcga.cohort[,pos]), 1), col = 'gray70')

  axis(side = 2, at = yt, labels = 10^yt, font = 2, las = 2, lwd = 2)
...
# }else{
#   tcga.cohort.gg = ggplot(data = tcga.cohort, aes(x = cohort, y = log_rate, group = Tumor_Sample_Barcode, color = TCGA))+
#     geom_point(position = position_jitter(width = 0.3), size = 0.2, alpha = 0.8)+
#     geom_point(data = tcga.cohort.med, aes(y = Median_Mutations, x = Cohort), shape = 95, inherit.aes = FALSE, color = medianCol, size = 6)+
#     cowplot::theme_cowplot(font_size = fontSize, line_size = 1)+
#     scale_y_continuous(breaks = seq(min(tcga.cohort$log_rate), max(tcga.cohort$log_rate)), labels = 10^(seq(min(tcga.cohort$log_rate), max(tcga.cohort$log_rate))))+
#     cowplot::background_grid(major = 'x')+
#     theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1, face = "bold"), legend.position = 'none', axis.text.y = element_text(face="bold", size = 10), axis.title.y = element_text(face="bold", size = 12))+
#     scale_color_manual(values = rev(col))+
#     ylab('log10 (Mutations per sample)')+xlab('')
