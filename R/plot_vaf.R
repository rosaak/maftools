...
plotVaf = function(maf, vafCol = NULL, genes = NULL, top = 10,
  ...
  if(is.null(genes)){
    genes = as.character(getGeneSummary(x =laml)[1:top, Hugo_Symbol])
  }
  ...
  if(flip){
    ...
    axis(side = 2, at = 1:length(genes), labels = b$names, tick = FALSE, las = 2, font = 2, line = -1, cex.axis = gene_fs)
    if(showN){
      axis(side = 4, at = 1:length(genes), labels = b$n, font = 2, tick = FALSE, line = -1, las = 2, cex.axis = gene_fs)
    }
    ...
    axis(side = 1, at = 1:length(genes), labels = b$names, tick = FALSE, las = 2, font = 2, line = -1, cex.axis = gene_fs)
    axis(side = 2, at = seq(0, 1, 0.2), las = 2, font = 2, lwd = 2, cex.axis = axis_fs)
    if(showN){
      axis(side = 3, at = 1:length(genes), labels = b$n, font = 2, tick = FALSE,
